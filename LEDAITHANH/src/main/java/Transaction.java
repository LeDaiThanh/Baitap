...
public class Transaction {
    ...
    public boolean processTransaction() {
        ...
        for(TransactionInput i : inputs) {
            i.UTXO = LDT_Blockchain.UTXOs.get(i.transactionOutputId);
        }

        //Kiểm tra giao dịch hợp lệ
        if(getInputsValue() < LDT_Blockchain.minimumTransaction) {
            System.out.println("Đầu vào của giao dịch quá bé: " + getInputsValue());
            System.out.println("Vui lòng nhập số tiền giao dịch lớn hơn " + LDT_Blockchain.minimumTransaction);
            return false;
        }
        ...
        for(TransactionOutput o : outputs) {
            LDT_Blockchain.UTXOs.put(o.id , o);
        }
        ...
        for(TransactionInput i : inputs) {
            ...
            LDT_Blockchain.UTXOs.remove(i.UTXO.id);
        }
